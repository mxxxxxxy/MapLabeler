<template>
  <div class="layer-manager" :style="{ height: expandedList ? height+'px' : '3em', width: width+'px', left: left+'px', top: top+'px' }">
    ...
    <div class="layer-list" v-show="expandedList">
      <ul>
        <li v-for="(layer, key) in layers" :key="key">
          <span class="icon">
            <i
              :class="getVisibilityIcon(layer.visibleKey)"
              @click="toggleVisibility(layer.visibleKey)"
            ></i>
          </span>
          <span class="layer-name">
            ...
            <i
              v-if="layer.children"
              :class="expandedLayers[key] ? 'fa fa-angle-down' : 'fa fa-angle-right'"
              @click="toggleExpand(key)"
            ></i>
          </span>
          <ul v-if="layer.children && expandedLayers[key]">
            <li
              v-for="(points, name) in layer.children"
              :key="name"
              :ref = "(el) => setElementRef(name, el)"
              :class="{ hovered: state.hoveredElement === name }"
              @mouseenter="hoverElement(name)"
...
const layers = computed(() => ({
  ...
  areas: { label: "区域", visibleKey: "areasVisible", children: data.areaCoordinates },
  paths: { label: "路径", visibleKey: "pathsVisible", children: data.pathData },
  points: { label: "地点", visibleKey: "pointsVisible", children: data.pointCoordinates },
}));
...
const setElementRef = (name, el) => {
  if (el) {
    elementRefs.value[name] = el;
  }
};
...
const toggleVisibility = (key) => (state[key] = !state[key]);
...
.layer-manager {
  ...
  backdrop-filter: blur(10px); /* 可选：增加模糊效果，使背景更柔和 */
